...
Luồng hoạt động mỗi vòng:
  ...
  2. Mỗi client tự huấn luyện, tạo gói (S, C, proof, Δw̃), rồi POST /goi.
...
if cfg.DUNG_WARMUP:
    ...
    delta_warm = huan_luyen_cuc_bo(mo_hinh_ban_dau, Xw, yw, so_lop,
...
@app.get("/du-lieu/<int:client_id>")
def lay_du_lieu(client_id):
    """Trả về dữ liệu RIÊNG của client này (mô phỏng: dữ liệu vốn nằm sẵn ở máy client)."""
    if not (0 <= client_id < cfg.SO_CLIENT):
        return jsonify({"loi": "client_id không hợp lệ"}), 400
    X, y = du_lieu["clients"][client_id]
